"""Standalone runner script for Stage 11 — PyTorch Geometric GCN Regressor Benchmark.

Evaluates PyG GCNRegressor across 109 subjects under Leave-One-Subject-Out (LOSO) CV
...
from graph.gcn_trainer import evaluate_gcn_loso


def append_to_ledger(ledger_path: Path, record: dict) -> None:
    """Append benchmark run record to master ledger CSV."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Stage 11 PyG GCN Regressor Benchmark.")
    ...
    logger.info("==================================================")
    logger.info("Stage 11: PyTorch Geometric GCN Regressor Benchmark")
    logger.info("Dataset: %s", graph_dir)
    logger.info("==================================================")

    start_time = time.perf_counter()
    res = evaluate_gcn_loso(graph_dir, cfg, out_dir, device_str=args.device)
    ...
    record = {
        ...
        "Hyperparameters": f"hidden={cfg['model']['hidden_channels']},layers={cfg['model']['num_layers']},lr={cfg['training']['lr']}",
    ...
    logger.info("GCN Benchmark complete in %.2fs. Ledger updated at %s", elapsed, ledger_path)
    logger.info("==================================================")
    logger.info("GCN vs SVR Comparison:")
    logger.info("GCN Performance: R2=%.4f, Pearson r=%.4f, MAE=%.4f", m["r2"], m["pearson_r"], m["mae"])
