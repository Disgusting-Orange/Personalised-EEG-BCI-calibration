"""PyTorch Geometric Graph Convolutional Network (GCN) Regressor for Subject-Level EEG Motor Imagery Regression.
"""
...
class GCNRegressor(nn.Module):
    """Publication-quality PyG Graph Convolutional Network Regressor."""
    ...
    def __init__(
        ...
        in_dim_pool = hidden_channels * multiplier
        if self.pooling_type == "attention":
            gate_nn = torch.nn.Sequential(
                torch.nn.Linear(in_dim_pool, hidden_channels // 2),
                torch.nn.ReLU(),
                torch.nn.Linear(hidden_channels // 2, 1),
            )
            self.att_pool = GlobalAttention(gate_nn)

        fc1_in = (in_dim_pool * 2) if pooling == "concat" else in_dim_pool
